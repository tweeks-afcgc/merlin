...
export function teamDisplayName(team: Team, seasons: Season[]): string {
  ...
  return age !== null ? `Under ${age} ${team.name}${nick}` : `${team.name}${nick}`
}
...
export function fixtureOpponentName(
  clubTeam: { name: string; clubs?: { name: string } | null; internal_team?: { name: string; type: string; founding_age_group: number | null; founding_season_id: string | null; nickname?: string | null } | null } | null,
  seasons: Season[],
  seasonId: string
): string {
  ...
  if (internalTeam) {
    const nick = internalTeam.nickname ? ` (${internalTeam.nickname})` : ''
    if (internalTeam.type === 'junior') {
      ...
      if (currentAge !== null && targetIndex !== -1 && currentIndex !== -1) {
        ...
        return `Under ${targetAge} ${internalTeam.name}${nick}`
      }
    }
    return clubTeam.name.replace(/^\[Internal\]\s*/, '') + nick
  }
  // External club team
...
export function teamDisplayNameForSeason(team: Team, seasons: Season[], seasonId: string): string {
  const nick = team.nickname ? ` (${team.nickname})` : ''
  if (team.type === 'senior') return `${team.name}${nick}`
  if (!team.founding_age_group || !team.founding_season_id) return `${team.name}${nick}`
  ...
  if (foundingIndex === -1 || targetIndex === -1) return `${team.name}${nick}`
