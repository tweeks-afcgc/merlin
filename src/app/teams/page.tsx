        ...
        <div className="space-y-3">
          {teams.map(team => (
            <div key={team.id} className="relative bg-white rounded-xl border border-gray-100 shadow-sm hover:shadow-md hover:border-red-200 transition group">
              <Link href={`/teams/${team.id}`} className="flex items-center gap-4 px-5 py-4">
                {/* Kit circle */}
                <div
              ...
              {profile?.role === 'admin' && (
                <Link
                  ...
                  className="absolute top-3 right-10 p-1.5 rounded-lg text-gray-300 hover:text-red-800 hover:bg-red-50 transition"
                  title="Edit team"
                >
