...
export default async function AdminVolunteersPage() {
  ...
  const [{ data: rawVolunteers }, { data: rawTeams }, { data: seasons }] = await Promise.all([
    supabase
      .from('volunteers')
      .select('id, first_name, last_name, email, is_app_user, user_role, is_referee, volunteer_roles(id, role_type, role_name, team_id, teams(id, name, type, founding_age_group, founding_season_id, age_group))')
  ...
  return (
    <AppShell userName={profile?.full_name ?? null} isAdmin>
      <div className="max-w-3xl mx-auto px-4 py-8">
        <AdminNav />
        <VolunteersClient volunteers={volunteers} teams={teams} />
      </div>
    </AppShell>
  )
}
